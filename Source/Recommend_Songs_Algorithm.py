...
def song_based_recommender(trainingFile, user_id, alpha, q, N):
    users_list = list(util.load_users("kaggle_users.txt"))

    # Order songs by popularity
    popular_songs_by_id = util.get_songs_by_popularity("msd_train.txt")

    #load users Index
    unique_users_list = util.load_unique_users(trainingFile)

    #Ennumerate userNames with index
    ...
    #callign with similarity measure as  cosine 
    ...
    user_song_map = util.user_song_map('kaggle_visible_evaluation_triplets.txt')
    ...
    out_put_filename = "songbased"+str(time.time())+".txt"
...
def user_based_recommender(alpha, q, user_id, N):
    users = util.load_users("kaggle_users.txt")
    # index the users
    ...
    # index the songs
    s_indices = util.song_index('kaggle_songs.txt')
    ...
    s_u_map = util.s_u_map("kaggle_visible.txt", u_indices, s_indices)
    #user to song map. user id is key and the song ids listened by the user are the values e.g. {1:[2,3,4],3:[4,5,6]}
    u_s_map = util.u_s_map("kaggle_visible.txt", u_indices, s_indices)
    #Getting songs by popularity
    popular_songs_by_id = util.get_songs_by_popularity("kaggle_visible.txt")
    ...
    #save results
    out_filename_2 = "userbased"+str(time.time())+".txt"
    util.save_results(top_recommended_songs, out_filename_2)
    return top_recommended_songs

def run_algorithms(alpha,q,N):
    users = util.load_users("kaggle_users.txt")
    total_users = len(users)
    trainingFile = "msd_train.txt"
    testFile = "kaggle_hidden.txt"
